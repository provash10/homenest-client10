...
const MyRatings = () => {
  ...
  const { user } = use(AuthContext);
   ...
   const userRatings = ratings.filter(
    (rating) => rating.reviewerEmail === user?.email
  );
    ...
    <div className="max-w-6xl mx-auto p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {userRatings.map(rate => (
          <div key={rate._id} className="p-4 shadow-md rounded-2xl border hover:shadow-lg transition bg-white">
            <img src={rate.propertyImage} className="w-full h-48 object-cover rounded-xl mb-3" alt={rate.propertyName}/>

            <h3 className="text-lg font-semibold mb-1">{rate.propertyName}</h3>
            <p className="text-yellow-500 font-medium"><FaStar /> {rate.rating}/5</p>
